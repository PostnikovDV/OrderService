...
public:
	HttpListener(
		...
		: ioc_(ioc), acceptor_(net::make_strand(ioc)), doc_root_(std::move(doc_root)), orderService_(std::move(order_service))
	{
		beast::error_code ec;
...
private:
	...
	void on_accept(beast::error_code ec, tcp::socket socket)
	{
		...
		{
			std::make_shared<HttpSession>(std::move(socket), doc_root_, orderService_)->run();
		}
